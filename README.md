...
```
### Now recompute PRS and R² by appling best p value threshold (from ```tune.prsice```) in target dataset for final evaluation ($\color{green}{out-of_sample}$ $\color{green}{prediction}$)
```
./PRSice_linux \
--a1 A1 \
--base /QRISdata/Q9427/ISG2026/ukb_matched.ma \
