...
--out target
```
Now please compare ```tune.prsice``` and ```target.prsice``` and you will see ~5% reduction of accuracy in target cohort. It is extected to be more pronounced if we use larger sample size.
